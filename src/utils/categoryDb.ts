...
export async function getCategories({ user_id }: { user_id?: string } = {}): Promise<StoredCategory[]> {
  ...
  let query =
    "SELECT id, user_id, label, type, is_system, synced, deleted, created_at FROM categories WHERE (deleted = 0 OR deleted IS NULL)";
  const params: string[] = [];
  if (user_id) {
...
export async function addCategory(data: {
  ...
  const id = generateId();
  db.run(
    "INSERT INTO categories (id, user_id, label, type, is_system, synced, deleted, created_at) VALUES (?, ?, ?, ?, 0, 0, 0, ?)",
    [id, data.user_id ?? null, data.label.trim(), data.type, new Date().toISOString()]
  );
  saveDB();
  return { id };
}
